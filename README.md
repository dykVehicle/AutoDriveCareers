...
### ✨ 用户数据服务器端持久化
- **服务器端存储**：支持将候选人和企业账户数据保存到服务器，不再因清除浏览器而丢失
- **多端同步**：可在不同设备间同步登录状态和数据
- **灵活配置**：支持本地存储和服务器存储两种模式，通过环境变量切换
- **向下兼容**：未配置服务器时仍可使用本地存储，完全向下兼容

### 📄 简历导出功能
- **导出PDF**：一键导出精美简历为PDF格式，支持完整排版和中文显示
- **导出Word**：一键导出简历为Word文档（DOCX），可二次编辑
- **自动命名**：导出文件自动包含姓名和日期
- **完整内容**：支持个人信息、技能、工作经历、项目、教育、证书、奖项等所有板块

📚 **详细说明**：
- [功能更新文档](./FEATURES_UPDATE.md) - 完整的功能说明和使用指南
- [后端API文档](./BACKEND_API.md) - 服务器端API接口规范和实现示例
